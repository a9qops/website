...
export default async function ServicesPage({params}: {params: Promise<{locale: string}>}) {
  ...
  const heading = content.heading || (locale === 'ar' ? 'ما بعد إنتاج\nيصنع الأثر' : 'POST-PRODUCTION\nTHAT MOVES');
  const introduction = content.introduction || (locale === 'ar'
    ? 'عملية ما بعد إنتاج مركّزة للأفلام والإعلانات والحملات الرقمية، من التجميع الأول حتى التسليم النهائي.'
    : 'A focused post-production process for films, commercials, and digital campaigns—from the first assembly to final delivery.');
            ...
            {services.map((service, idx) => {
              ...
              >
                <div className="flex items-start gap-8 md:gap-16">
                  ...
                  <div>
                    <h3 className="text-2xl md:text-4xl font-heading font-bold uppercase text-white group-hover:text-zinc-300 transition-colors tracking-tight">
                      {title}
                    </h3>
                    <p className="mt-4 max-w-2xl whitespace-pre-line text-base leading-relaxed text-zinc-400 md:text-lg">
                      {description}
                    </p>
                  </div>
                </div>
